 ...
 * @typedef {('asm'|'bash'|'bf'|'c'|'cobol'|'cpp'|'cs'|'css'|'csv'|'diff'|'docker'|'gd'|'git'|'go'|'html'|'http'|'ini'|'java'|'js'|'jsdoc'|'json'|'leanpub-md'|'log'|'lua'|'make'|'md'|'pl'|'plain'|'py'|'regex'|'rs'|'sql'|'todo'|'toml'|'ts'|'uri'|'xml'|'yaml')} ShjLanguage
 */

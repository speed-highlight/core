 ...
 * @typedef {('asm'|'bash'|'bf'|'c'|'cobol'|'cpp'|'cs'|'css'|'csv'|'diff'|'docker'|'gd'|'git'|'go'|'html'|'http'|'ini'|'java'|'js'|'jsdoc'|'json'|'leanpub-md'|'log'|'lua'|'make'|'md'|'mongodb'|'php'|'pl'|'plain'|'ps1'|'py'|'regex'|'rs'|'sql'|'todo'|'toml'|'ts'|'uri'|'xml'|'yaml')} ShjLanguage
 */
